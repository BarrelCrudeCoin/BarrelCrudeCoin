...
const char * const BITCOIN_CONF_FILENAME = "barrelcrudecoin.conf";
...
#else
    const char* pszModule = "barrelcrudecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BarrelCrudeCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BarrelCrudeCoin";
#else
    // Unix
    return pathRet / ".barrelcrudecoin";
#endif
#endif
}
