 ...
 */
static const char* BITCOIN_PID_FILENAME = "barrelcrudecoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("barrelcrudecoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/BarrelCrudeCoin/BarrelCrudeCoin>";
    const std::string URL_WEBSITE = "<https://barrelcrudecoin.com>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("barrelcrudecoin-loadblk");
...
//        return; // Nothing to read, so just return
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if barrelcrudecoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if barrelcrudecoin is started while in a temporary directory.\n",
